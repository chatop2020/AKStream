...
namespace SIPSorcery.SIP.App
{
    public class SIPRegistrationUserAgent
    {
        ...
        public void Start()
        {
            ...
            logger.LogDebug(
                $"Starting SIPRegistrationUserAgent for {m_sipAccountAOR}, callback period {callbackPeriod}ms.");
        ...
        private void SendInitialRegister()
        {
            try
            {
                ...
                {
                    logger.LogWarning("Registration to " + m_sipAccountAOR.ToString() +
                ...
                else
                {
                    ...
                    if (registrarSIPEndPoint == null)
                    {
                        ...
                        var lookupResult = m_sipTransport.ResolveSIPUriAsync(uri).ConfigureAwait(false).GetAwaiter()
                            .GetResult();
                    ...
                    {
                        logger.LogDebug("Initiating registration to " + m_registrarHost + " at " +
                                        registrarSIPEndPoint.ToString() + " for " + m_sipAccountAOR.ToString() + ".");
